...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        for fold_idx, (t_idx, v_idx) in enumerate(kf.split(train_g)):
            ...
            model = TriangleGAT(in_dim=26).to(DEVICE)
            best, hist = train_one_fold(model, tg, vg, cw, BS, f"S{seed}F{fold_idx+1}", WARMUP, EPOCHS)
            all_results.append({"seed": seed, "fold": fold_idx+1, "best_val": float(best)})
            print(f"  Best: {best:.4f}")
            torch.save(model.state_dict(), MODEL_DIR / f"model_s{seed}_f{fold_idx+1}.pt")
            del model; torch.cuda.empty_cache()

        # Final model on all train data
        print(f"\n  Final model seed={seed}")
        fm = TriangleGAT(in_dim=26).to(DEVICE)
        opt = torch.optim.AdamW(fm.parameters(), lr=LR, weight_decay=1e-4)
        sc = warmup_cosine_scheduler(opt, WARMUP, EPOCHS)
        n_all = len(train_g)
        for epoch in range(1, EPOCHS+1):
            ...
            for i in range(0, n_all, BS):
                idx = perm[i:i+BS].tolist()
                batch = Batch.from_data_list([train_g[j] for j in idx])
                opt.zero_grad()
                with torch.amp.autocast('cuda'):
                ...
                scaler.step(opt); scaler.update()
            sc.step()
        torch.save(fm.state_dict(), MODEL_DIR / f"model_{seed}.pt")
        print(f"  Saved model_{seed}.pt")
    ...
    for seed in [42, 123, 456]:
        ...
        m.load_state_dict(torch.load(MODEL_DIR / f"model_{seed}.pt", map_location=DEVICE))
    ...
    print(f"\nTest Acc (3-model ensemble): {test_acc:.4f}")
    print(f"Per-class:")
    ...
    report = {
        ...
        "train_time_s": round(time.time() - t0, 0)
    }
    with open(MODEL_DIR / "ensemble_report.json", "w") as f:
        json.dump(report, f, indent=2)
    print(f"\nReport saved. Total time: {(time.time()-t0)/60:.1f}min")
